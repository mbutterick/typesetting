...
(test-module
 ...
 (check-equal?
  (let ()
    ...
    (send loca encode es '#hash((version . 0) (offsets . (0 76 156))))
    (send es dump)) #"\0\0\0L\0\234")
 (check-equal?
  (let ()
    (define es (+EncodeStream))
    (send loca encode es '#hash((version . 1) (offsets . (0 76 156))))
    (send es dump)) #"\0\0\0\0\0\0\0L\0\0\0\234")
